...
def trade(first, second):
    ...
    while (m<len(first)-1 and n<len(second)-1) and sum(first[:m])!=sum(second[:n]):
...
def make_glookup(class_assignments):
    ...
    sumcredit = 0
    getcredit = 0
    def f(section, credit):
        assert section in class_assignments, "invalid assignment name"
        nonlocal sumcredit, getcredit
        getcredit += credit
        sumcredit += class_assignments[section]
        return getcredit/sumcredit
    return f

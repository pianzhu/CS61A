...
def g_iter(n):
    ...
    res = [1, 2, 3]
    for i in range(3, n):
        res.append(res[i-1] + 2*res[i-2] + 3*res[i-3])
    return res[n-1]
...
def missing_digits(n):
    ...
    if(n<10):
        return 0
    miss = 0 if (n%10-(n%100//10) <= 1) else n%10-(n%100//10)-1
    return miss + missing_digits(n//10)
...
def count_change(total):
    ...
    def change_helper(total, min_coin):
        if (total == 0):
            ...
            return change_helper(total, min_coin<<1) + change_helper(total-min_coin, min_coin)

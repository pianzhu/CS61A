...
def balanced(m):
    ...
    left_arm = left(m)
    right_arm = right(m)
    if (total_weight(left_arm[2])*length(left_arm) == total_weight(right_arm[2])*length(right_arm) and balanced(left_arm[2]) and balanced(right_arm[2])):
        return  True
    return False
...
def preorder(t):
    ...
    if is_leaf(t):
        ...
        return [label(t)] + res
        """
          another solution: source by FlyingPig
...
def div_interval(x, y):
    ...
    reciprocal of y."""
    "*** YOUR CODE HERE ***"
    assert upper_bound(y) * lower_bound(y) > 0, 'Error div'
...
def quadratic(x, a, b, c):
    ...
    "*** YOUR CODE HERE ***"
    f = b / (-2 * a)
    if f < lower_bound(x) or f > upper_bound(x):
